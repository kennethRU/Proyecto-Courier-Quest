...
# Estas clases deben estar definidas en models.py
...
class Game:
    def __init__(self) -> None:
        ...
        self._load_assets() # Cargar imágenes y otros recursos
    ...
    def _init_weather(self, raw) -> WeatherState:
        ...
        return WeatherState(
            ...
            bursts=bursts,   # <-- guardar todos los bursts
            burst_index=0    # <-- índice del burst actual
        )


    # --------- Loop principal ---------
    def run(self) -> None: # [cite: 196]
        running = True # [cite: 196]
        while running: # [cite: 196]
            dt = self.clock.tick(TARGET_FPS) / 1000.0 # [cite: 196]
            for event in pygame.event.get(): # [cite: 196]
                if event.type == pygame.QUIT: # [cite: 196]
                    running = False # [cite: 197]
                elif event.type == pygame.KEYDOWN: # [cite: 197]
                    self._on_keydown(event.key) # [cite: 197]

            if not self.state.game_over: # [cite: 197]
                self._update(dt) # [cite: 197]
            self._draw() # [cite: 197]

        pygame.quit() # [cite: 197]
    ...
    def _update(self, dt: float) -> None:
        ...
        if (GAME_DURATION_SECONDS - self.state.elapsed) <= 0:
            # Al terminar tiempo, si meta no se logro es derrota, si se alcanzo, victoria.
    ...
    def _update_weather(self, dt: float) -> None:
        ...
        if w.transition_time_left > 0:
            ...
            if w.burst_time_left <= 0:
                # Usar siguiente burst de la API
                if hasattr(w, "bursts") and w.bursts and w.burst_index + 1 < len(w.bursts):
                    ...
                    # Si ya no hay más bursts, repetir último
                    w.target = w.current
    ...
    def _handle_input_movement(self, dt: float) -> None:
        ...
        if 0 <= tx < self.state.city.width and 0 <= ty < self.state.city.height:
            if not self.state.city.is_blocked(tx, ty):
                ...
                # --- Recalcular camino al destino actual ---
                self._recalculate_path()

    def _recalculate_path(self) -> None:
        """Calcula el camino más corto hasta el destino del pedido actual,
        evitando calles bloqueadas, edificios y parques."""
        ...
        # --- Crear grid bloqueado por edificios y parques ---
        grid = [
            [0 if not self._is_blocked((x, y)) else 1 for x in range(w)]
            for y in range(h)
        ]

        # --- Buscar la celda accesible más cercana al destino si está bloqueada ---
    ...
    def _try_accept_job_here(self) -> None:
        ...
        for job in self.state.jobs_active:
            if not job.accepted:
                ...
                if dist <= 1:   # mismo lugar o adyacente
                    job.accepted = True
        ...
        # Si llegamos aquí, no hay pedido para aceptar
    ...
    def _try_deliver_current(self) -> None:
        ...
        if dist <= 1:
            ...
            # TERMINAR JUEGO si se entregaron todos los pedidos
    ...
    def _cancel_current(self) -> None:
        ...
        # Marcar como cancelado y restar reputación
        job.canceled = True
    ...
    def _check_fail_conditions(self) -> None:
        ...
        # Derrota si se acabó el tiempo y no se alcanzó la meta ni todos los pedidos entregados
    ...
    def _draw_map(self, surface: pygame.Surface) -> None:
        ...
        COLOR_STREET = (180, 180, 180)  # Gris claro para Calle ('C')
        
        drawn_building_tiles = set() # Usado para evitar procesar el mismo tile 'B' dos veces
        ...
        for y in range(city.height):
            for x in range(city.width):
                ...
                # DIBUJO DE BLOQUES GRANDES ('B' o 'P')
    ...
    def _draw_controls_legend(self) -> None:
        ...
        # --- Leyenda de colores (arriba del panel) ---
        legend_x = rect_x + 10
        y_offset = rect_y + 20
        ...
        y_offset += 40  # Espacio extra antes de los controles
    ...
    def _apply_expire_penalty(self, job) -> None:
        """Aplica penalización de reputación si un pedido expira sin ser entregado."""
    ...
    def _load_assets(self) -> None:
        """Carga y escala todos los activos de imagen del juego una sola vez."""
        try:
            ...
            # Cargar imagen del distrito de edificios
            self.building_district_img = pygame.image.load("assets/building.png").convert_alpha()
            
            # NUEVO: Cargar imagen del distrito de parques
